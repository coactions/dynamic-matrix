...
def main() -> None:
    ...
    try:
        ...
        core.debug(f"Generated {len(result)} matrix entries.")
        names = [k["name"] for k in result]
        core.debug(f"Job names: {', '.join(names)}")

        matrix_include = f"Hello {result}"

        core.set_output("matrix_include", matrix_include)
